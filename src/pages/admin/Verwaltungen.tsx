...
export default function AdminVerwaltungen() {
  ...
  return (
    <DashboardLayout basePath="/admin/dashboard">
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 bg-hp-black text-white text-sm font-body
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 font-body mt-0.5">
            Verwaltungsunternehmen pflegen und Immobilien zuweisen
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <button
            onClick={openCreate}
            className="mt-4 px-4 py-2 rounded-xl text-sm font-semibold font-body text-white
          ...
          {list.map(v => (
            <div key={v.id}
              ...
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-full bg-hp-black flex items-center justify-center
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {v.address_city && (
                    <p className="text-xs text-gray-400 font-body mt-0.5">{v.address_city}</p>
                  )}
                </div>
              </div>
                ...
                {(v.address_street || v.address_city) && (
                  <a href={mapsUrl(v)} target="_blank" rel="noopener noreferrer"
                    ...
                    <span className="truncate">
                      {[v.address_street, v.address_zip, v.address_city].filter(Boolean).join(', ')}
                    </span>
                  </a>
                )}
                {v.website && (
                  <a href={v.website.startsWith('http') ? v.website : `https://${v.website}`}
                     ...
                     className="flex items-center gap-2 text-gray-600 hover:text-hp-highlight transition-colors truncate">
                    <span>🌐</span><span className="truncate">{v.website}</span>
                  </a>
                )}
              </div>
              ...
              <div className="flex gap-2 pt-2 border-t border-gray-100">
                <button
                  onClick={() => openEdit(v)}
                  className="flex-1 py-1.5 rounded-lg border border-gray-200 text-xs font-semibold
                             text-gray-600 font-body hover:border-hp-highlight hover:text-hp-highlight transition-colors">
                  ✏️ Bearbeiten
                </button>
                <button
                  onClick={() => setDeleteId(v.id)}
                  className="px-3 py-1.5 rounded-lg border border-red-100 text-xs font-semibold
                             text-red-400 font-body hover:border-red-300 hover:text-red-600 transition-colors">
                  ✕
                </button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg my-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="text-base font-bold text-hp-black" style={{ fontFamily: 'var(--font-heading)' }}>
                {editing ? 'Verwaltung bearbeiten' : 'Neue Verwaltung'}
              </h2>
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              {/* Name */}
              <div>
                <label className="block text-xs text-gray-500 font-body mb-1 font-semibold">
                  Name *
                </label>
                <input
                  autoFocus
              ...
              <div className="space-y-2">
                <p className="text-xs text-gray-500 font-body font-semibold">Adresse</p>
                <input
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  />
                  <input
                    className={`${inputCls} col-span-2`} style={focusRing()}
                    placeholder="Stadt"
              ...
              {/* Kontakt */}
              <div className="space-y-2">
                <p className="text-xs text-gray-500 font-body font-semibold">Kontakt</p>
                <input
                  className={inputCls} style={focusRing()}
                  placeholder="Telefon"
                  type="tel"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <p className="text-sm text-gray-500 font-body">
              Immobilien die dieser Verwaltung zugewiesen sind, werden nicht gelöscht – lediglich die Zuweisung wird aufgehoben.
            </p>
