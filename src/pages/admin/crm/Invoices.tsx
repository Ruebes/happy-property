...
export default function Invoices() {
  ...
  const setStatus = async (inv: Row, status: 'paid' | 'canceled' | 'sent') => {
    ...
    showToast(t('invoices.statusUpdatedToast', '{{number}}: {{status}}', { number: inv.invoice_number, status: INVOICE_STATUS_LABEL[status] }))
    fetchAll()
  }

  const copyLink = (inv: Row) => {
    void navigator.clipboard.writeText(`${window.location.origin}/re/${inv.token}`)
    showToast(t('invoices.linkCopiedToast', 'Link zu {{number}} kopiert', { number: inv.invoice_number }))
  }
    ...
    <DashboardLayout basePath="/admin/crm">
      ...
      <div className="p-6 space-y-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t('invoices.pageTitle', 'Rechnungen')}</h1>
            <p className="text-sm text-gray-500 mt-0.5">{t('invoices.pageSubtitle', 'Rechnungen erstellen, herunterladen und versenden. Aussteller: sveru ltd (Happy Property).')}</p>
          </div>
          <button onClick={() => setCreating(true)}
            className="px-3 py-1.5 rounded-xl text-white text-sm font-medium whitespace-nowrap" style={{ backgroundColor: '#ff795d' }}>
            {t('invoices.newInvoiceBtn', '+ Neue Rechnung')}
          </button>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-400 text-center py-12 bg-white rounded-2xl border border-dashed border-gray-200">{t('invoices.emptyState', 'Noch keine Rechnungen.')}</p>
        ) : (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              <thead><tr className="text-left text-xs text-gray-400 border-b border-gray-100">
                <th className="px-4 py-3 font-medium">{t('invoices.colNumber', 'Nummer')}</th>
                <th className="px-4 py-3 font-medium">{t('invoices.colDate', 'Datum')}</th>
                <th className="px-4 py-3 font-medium">{t('invoices.colRecipient', 'Empfänger')}</th>
                <th className="px-4 py-3 font-medium text-right">{t('invoices.colAmount', 'Betrag')}</th>
                <th className="px-4 py-3 font-medium">{t('invoices.colStatus', 'Status')}</th>
                <th className="px-4 py-3 font-medium text-right">{t('invoices.colActions', 'Aktionen')}</th>
              </tr></thead>
              <tbody>
                {rows.map(inv => (
...
function ManualInvoiceModal({ onClose, onCreated }: { onClose: () => void; onCreated: (m: string) => void }) {
  ...
  const catalogOptions = [
    ...
    ...plans.map(p => ({ value: `p:${p.id}`, label: t('invoices.catalogSubscriptionLabel', 'Abo: {{name}} ({{price}})', { name: p.name, price: eurFmt(p.net_price) }) })),
  ]
  const addFromCatalog = (val: string) => {
    ...
    if (val.startsWith('p:')) { const p = plans.find(x => x.id === val.slice(2)); if (p) setLines(prev => [...prev, { description: t('invoices.subscriptionLineDescription', '{{name}} (Abo)', { name: p.name }), quantity: 1, unit_price_net: p.net_price }]) }
  }

  const create = async () => {
    const valid = lines.filter(l => l.description.trim() && l.unit_price_net > 0)
    if (!customerId) { setErr(t('invoices.errNoRecipient', 'Bitte einen Empfänger wählen.')); return }
    if (!valid.length) { setErr(t('invoices.errNoLineItems', 'Mindestens ein Posten mit Beschreibung + Betrag.')); return }
    setBusy(true); setErr('')
    try {
      ...
      if (!res?.ok) throw new Error(res?.error || t('invoices.errGeneric', 'Fehler'))
      onCreated(res.sent
        ? t('invoices.createdAndSentToast', 'Rechnung {{number}} erstellt & gesendet', { number: res.invoice_number })
        : t('invoices.createdToast', 'Rechnung {{number}} erstellt', { number: res.invoice_number }))
    ...
    <div className="fixed inset-0 z-[60] flex items-start justify-center bg-black/40 p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-6 max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">{t('invoices.modalTitle', 'Neue Rechnung')}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none">✕</button>
        </div>
        <div className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">{t('invoices.recipientLabel', 'Empfänger')}</label>
              <CustomSelect value={customerId}
                ...
                options={customers.map(c => ({ value: c.id, label: c.company_name, hint: c.vat_number ?? undefined }))} placeholder={t('invoices.recipientPlaceholder', '– Kunde wählen –')} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">{t('invoices.vatTreatmentLabel', 'MwSt-Behandlung')}</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-medium text-gray-500">{t('invoices.lineItemsLabel', 'Posten')}</label>
              {catalogOptions.length > 0 && (
                <div className="w-56"><CustomSelect value="" onChange={addFromCatalog} options={catalogOptions} placeholder={t('invoices.catalogPlaceholder', '+ aus Katalog')} /></div>
              )}
            </div>
            <div className="space-y-2">
              {lines.map((l, i) => (
                <div key={i} className="flex gap-2 items-start">
                  <input className={`${inputCls} flex-1`} placeholder={t('invoices.descriptionPlaceholder', 'Beschreibung')} value={l.description} onChange={e => setLine(i, { description: e.target.value })} />
            ...
            <button onClick={addLine} className="mt-2 text-xs text-orange-600 font-medium hover:text-orange-800">{t('invoices.addLineItemBtn', '+ Posten hinzufügen')}</button>
          </div>

          {/* Summen */}
          <div className="rounded-xl bg-gray-50 px-4 py-3 text-sm space-y-1">
            <div className="flex justify-between text-gray-600"><span>{t('invoices.subtotalLabel', 'Zwischensumme (netto)')}</span><span className="font-medium text-gray-800">{eurFmt(totals.subtotal_net)}</span></div>
            <div className="flex justify-between text-gray-600"><span>{info.rate > 0 ? t('invoices.vatRateLabel', 'MwSt {{rate}}%', { rate: info.rate }) : info.label}</span><span className="font-medium text-gray-800">{eurFmt(totals.vat_amount)}</span></div>
            <div className="flex justify-between pt-1.5 mt-1 border-t border-gray-200"><span className="font-semibold">{t('invoices.totalLabel', 'Gesamt')}</span><span className="font-bold" style={{ color: '#ff795d' }}>{eurFmt(totals.total_gross)}</span></div>
          </div>
          ...
          <label className="flex items-center gap-2.5 cursor-pointer select-none">
            ...
            <span className="text-sm text-gray-700">
              {customer?.email
                ? t('invoices.sendByEmailWithAddress', 'Direkt per E-Mail senden an {{email}}', { email: customer.email })
                : t('invoices.sendByEmail', 'Direkt per E-Mail senden')}
            </span>
          </label>
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100">
          ...
          <button onClick={create} disabled={busy} className="px-5 py-2 rounded-lg text-white text-sm font-medium disabled:opacity-50" style={{ backgroundColor: '#ff795d' }}>
            {busy ? t('invoices.creatingBtn', 'Erstelle…') : send ? t('invoices.createAndSendBtn', 'Erstellen & senden') : t('invoices.createBtn', 'Erstellen')}
          </button>
        </div>
      </div>
    </div>
