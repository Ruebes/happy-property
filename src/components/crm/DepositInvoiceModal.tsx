...
export default function DepositInvoiceModal({ deal, onClose, onDone }: Props) {
  ...
  const defaultDesc = t('depositInvoiceModal.leadGenDescription', 'Leadgenerierung {{details}}', { details: [fullName, developer, project, unit].filter(Boolean).join(', ') }).trim()
  ...
  async function submit(send: boolean) {
    if (net <= 0) { setErr(t('depositInvoiceModal.errNetAmountRequired', 'Bitte einen Netto-Betrag größer 0 eingeben.')); return }
    setBusy(true); setErr('')
    try {
      ...
      await supabase.from('activities').insert({
        ...
        content: t('depositInvoiceModal.activityContent', '{{number}} über {{amount}} ({{description}}){{statusSuffix}}', {
          number: res.invoice_number,
          amount: eurFmt(totals.total_gross),
  ...
  const recipient = customer
    ? `${customer.company_name}${customer.contact_name ? t('depositInvoiceModal.attentionOfSuffix', ' (z. Hd. {{name}})', { name: customer.contact_name }) : ''}`
    : '—'
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h3 className="font-heading text-lg text-hp-black">{t('depositInvoiceModal.title', 'Anzahlungs-Rechnung')}</h3>
            <p className="text-xs text-gray-500 mt-0.5">{fullName || t('depositInvoiceModal.leadFallback', 'Lead')}{project ? ` · ${project}${unit ? ` ${unit}` : ''}` : ''}</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none">×</button>
        </div>

        {loading ? (
          <div className="p-10 text-center text-gray-400 text-sm">{t('depositInvoiceModal.loading', 'Lädt…')}</div>
        ) : (
          <div className="px-6 py-5 space-y-4">
            {/* Empfänger */}
            <div className="bg-gray-50 rounded-xl px-4 py-3 text-sm">
              <span className="text-gray-500">{t('depositInvoiceModal.recipientLabel', 'Empfänger: ')}</span>
            ...
            {/* Posten */}
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">{t('depositInvoiceModal.lineItemLabel', 'Rechnungsposten')}</label>
            ...
            {/* Netto-Betrag */}
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">{t('depositInvoiceModal.netAmountLabel', 'Netto-Betrag')}</label>
              <NumberStepper value={net} onChange={setNet} step={100} min={0} suffix="€" />
            </div>

            {/* MwSt-Behandlung */}
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">{t('depositInvoiceModal.vatTreatmentLabel', 'MwSt-Behandlung')}</label>
            ...
            <div className="rounded-xl border border-gray-200 overflow-hidden">
              <div className="bg-hp-black text-white text-[11px] font-semibold tracking-wide px-4 py-2">{t('depositInvoiceModal.previewBadge', 'VORSCHAU')}</div>
              <div className="px-4 py-3 space-y-1.5 text-sm">
                <Row k={t('depositInvoiceModal.subtotalNetLabel', 'Zwischensumme (netto)')} v={eurFmt(totals.subtotal_net)} />
                <Row k={info.rate > 0 ? t('depositInvoiceModal.vatRateLabel', 'MwSt {{rate}}%', { rate: info.rate }) : info.label} v={eurFmt(totals.vat_amount)} />
                <div className="flex justify-between items-center pt-2 mt-1 border-t border-gray-200">
                  <span className="font-semibold text-hp-black">{t('depositInvoiceModal.totalGrossLabel', 'Gesamtbetrag')}</span>
            ...
            <div className="flex gap-2 pt-1">
              <button disabled={busy || net <= 0} onClick={() => submit(false)}
                ...
                {t('depositInvoiceModal.draftOnlyButton', 'Nur Entwurf')}
              </button>
              <button disabled={busy || net <= 0} onClick={() => submit(true)}
                ...
                {busy
                  ? t('depositInvoiceModal.creatingButton', 'Erstelle…')
                  : recipientEmail
                    ? t('depositInvoiceModal.createAndSendButton', 'Erstellen & an {{name}} senden', { name: customer?.contact_name || t('depositInvoiceModal.customerFallback', 'Kunde') })
                    : t('depositInvoiceModal.createButton', 'Erstellen')}
              </button>
            </div>
            <p className="text-[11px] text-gray-400 text-center">
              {t('depositInvoiceModal.nextNumberHint', 'Rechnungsnummer wird automatisch vergeben (nächste: {{number}}).', { number: settings ? `${settings.invoice_prefix}${settings.next_number}` : '—' })}
            </p>
          </div>
        )}
      </div>
