...
function stylePrompt(title: string, rooms: string[], outdoor: string[], floors: Array<{ locator: string; label: string; refNr?: number }>, planLocator: string, issues?: string[]): string {
  ...
  return [
    ...
    `Room labels in German, uppercase, small elegant sans-serif, each label exactly once per room, with real German umlauts (Ä, Ö, Ü, ß — e.g. KÜCHE, ÜBERDACHTE VERANDA; never AE/OE/UE): ${rooms.join(', ')}.`,
