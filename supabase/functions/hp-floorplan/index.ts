...
function stylePrompt(title: string, rooms: string[], outdoor: string[], planLocator: string, issues?: string[]): string {
  const loc = planLocator
    ? `The reference sheet may contain several plans and a title block. Use ONLY this plan: ${planLocator}. Ignore everything else on the sheet.`
  ...
  return [
    ...
    ...(issues?.length ? [`FIX THESE ERRORS from the previous attempt (everything else stays identical): ${issues.join(' | ')}`] : []),
  ].join(' ')
}
...
async function generateStyled(store: HfStore, refId: string, prompt: string): Promise<Uint8Array> {
  let lastErr = ''
  for (const jt of JOB_TYPES) {
    try {
      return await hfGenerateBytes(store, jt, { prompt, aspect_ratio: '4:3', image_references: [{ id: refId }] })
    } catch (e) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.source_url) candidates.push(body.source_url)
      ...
      if (Array.isArray(fps)) for (const f of fps) {
    ...
    const cand = [...new Set(candidates)].slice(0, 5)
    ...
    const run = async () => {
      try {
        ...
        const imgs = await Promise.all(cand.map(u => fetchB64(shrink(u))))
        const unitDesc = [
        ...
        const analysis = await claude([ANALYZE_TOOL], 'emit_analysis', [
          ...
          { type: 'text', text: `Projekt „${projName}". ${unitDesc}.\nFinde das Kandidatenbild + den Plan darauf, der GENAU diese Wohnung (bzw. ihren Haustyp) zeigt. Lies Raumliste und — falls im Plan lesbar — reale Masse ab. Nichts schaetzen, nichts erfinden.` },
        ]) as { source_index?: number; plan_locator?: string; rooms?: Array<{ german?: string }>; outdoor?: string[]; dims?: string[]; floor_label?: string; confidence?: string; note?: string }

        const si = Number(analysis.source_index)
        if (!(si >= 0 && si < cand.length)) throw new Error(`Kein Kandidat zeigt den Plan von ${body.unit_number}${analysis.note ? ` (${analysis.note})` : ''}`)
        const srcUrl = cand[si]
        ...
        const title = `${projName.toUpperCase()} — ${titleUnit}${analysis.floor_label ? ` · ${analysis.floor_label}` : ''}`

        // ── 2) Higgsfield-Restyle (max. 2 Versuche mit Vision-Verifikation) ─────
        const store = hfStoreFrom(sb)
        const srcBytes = new Uint8Array(await (await fetch(srcUrl)).arrayBuffer())
        const srcMime = srcUrl.toLowerCase().includes('.png') ? 'image/png' : 'image/jpeg'
        const refId = await hfUploadImage(store, srcBytes, srcMime)

        let outBytes: Uint8Array | null = null
        let verified = false
        let issues: string[] = []
        for (let attempt = 0; attempt < 2; attempt++) {
          const bytes = await generateStyled(store, refId, stylePrompt(title, rooms, outdoor, analysis.plan_locator ?? '', attempt ? issues : undefined))
          ...
          const verdict = await claude([VERIFY_TOOL], 'emit_verdict', [
            ...
            { type: 'text', text: `Pruefe NUR den Plan „${analysis.plan_locator || 'auf dem Original'}" gegen Bild 2: Stimmen Waende, Tueroeffnungen samt Schwenkrichtung, Fenster, Treppe und Raumproportionen ueberein? Wurde etwas erfunden oder weggelassen? Stil/Farben/fehlende Masse sind KEINE Fehler.` },
        ...
        const note = noteParts.join(' · ').replace(/\.\s·/g, ' ·')
