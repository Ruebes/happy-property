...
//
// Karussell nutzt ECHTE Projektfotos aus crm_projects.images (keine KI-Bilder).
  ...
  try {
    ...
    if (mode === 'generate') {
      ...
      const { data: projects } = await supabase.from('crm_projects')
        .select('name, images').not('images', 'is', null)
      const projectInfo = ((projects ?? []) as { name: string; images: string[] }[])
        .filter(p => p.images?.length)
        .map(p => `- ${p.name}: ${p.images.length} Fotos [${p.images.join(' | ')}]`).join('\n')
...
Verfügbare Projekte mit ECHTEN Fotos (für Karussells IMMER diese echten Foto-URLs verwenden; „Luma" ist der Bauträger der Projekte Genesis, Emerald Park und Skala):
...
Antworte NUR mit JSON:
{
  ...
  "cards": [NUR bei carousel, 2-6 Karten: {"title": "max. 35 Zeichen", "description": "max. 60 Zeichen", "image_url": "eine der echten Projekt-Foto-URLs"}]
      ...
      if (plan.format === 'single') {
        ...
        if (!draft.cards.length) throw new Error('Karussell ohne Karten — bitte Auftrag präzisieren')
      }
